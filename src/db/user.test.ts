...
describe("UserRepository", () => {
  ...
  describe("findVerifiedWithUsage", () => {
    let testUserIds: number[] = [];
    ...
    beforeEach(async () => {
      const users = await knex("users")
        .insert([
          ...
          {
            name: "Idle Verified",
            email: "idle-verified@example.com",
            verified: true,
            api_call_count: 0,
          },
          {
    ...
    it("should return only verified users with api_call_count > 0", async () => {
      const users = await userRepository.findVerifiedWithUsage();
      const testUsers = users.filter((u) => testUserIds.includes(u.id));

      expect(testUsers).toHaveLength(1);
      expect(testUsers[0].email).toBe("active-verified@example.com");
    });

    it("should exclude verified users at exactly 0", async () => {
      const users = await userRepository.findVerifiedWithUsage();
      const emails = users.map((u) => u.email);

      expect(emails).not.toContain("idle-verified@example.com");
    });

    it("should exclude unverified users even when they have usage", async () => {
      const users = await userRepository.findVerifiedWithUsage();
      const emails = users.map((u) => u.email);

      expect(emails).not.toContain("active-unverified@example.com");
    });
  });

  describe("resetAllApiCallCounts", () => {
    let testUserIds: number[] = [];
    ...
    it("should reset verified users with usage to 0", async () => {
      await userRepository.resetAllApiCallCounts();

      const active = await knex("users").where({ email: "reset-active@example.com" }).first();
      expect(active?.api_call_count).toBe(0);
    });

    it("should not touch unverified users", async () => {
      await userRepository.resetAllApiCallCounts();

      const unverified = await knex("users")
        .where({ email: "reset-unverified@example.com" })
        .first();
      expect(unverified?.api_call_count).toBe(50);
    });

    it("should leave already-zero rows at 0", async () => {
      await userRepository.resetAllApiCallCounts();

      const idle = await knex("users").where({ email: "reset-idle@example.com" }).first();
      expect(idle?.api_call_count).toBe(0);
    });
  });
});
