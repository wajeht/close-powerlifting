// Per-row CSV → typed-value conversions. Each helper is small and pure so
// the loader stays a flat sequence of "read column → coerce → assign".
...
// `rowIdx` is passed so callers get stable, non-colliding slugs across
// re-ingests for the same row.
...
// Result of converting one CSV row. The loader uses this to dedupe lifters
// and meets (via `lifterUsername` and `meetPath`) before building the final
// Entry with its resolved FKs.
export interface NormalizedRow {
  lifterUsername: string;
  lifterName: string;
  meetPath: string;
  meet: Omit<Meet, "path"> & { path: string };
  // Entry without lifter_id / meet_id — the loader assigns those once it
  // has materialised the lifter and meet entities.
...
// Re-exported so consumers don't need to import the Lifter type just to
// construct one.
export function makeLifter(username: string, name: string): Lifter {
