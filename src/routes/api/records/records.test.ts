...
describe("GET /api/records", () => {
  ...
  test("should return records data with correct structure", async () => {
  ...
  test("should return array of record categories", async () => {
    const response = await authenticatedRequest().get("/api/records");
  ...
  test("should return record categories with title and records array", async () => {
    const response = await authenticatedRequest().get("/api/records");
  ...
  test("should return individual records with lifter information", async () => {
    const response = await authenticatedRequest().get("/api/records");
    const category = response.body.data[0];
...
describe("GET /api/records/:equipment", () => {
  ...
  test("should filter records by raw equipment", async () => {
  ...
  test("should return records with correct structure when filtered", async () => {
    const response = await authenticatedRequest().get("/api/records/raw");
    const category = response.body.data[0];
...
describe("GET /api/records/:equipment/:sex", () => {
  ...
  test("should filter records by equipment and sex (men)", async () => {
  ...
  test("should filter records by equipment and sex (women)", async () => {
  ...
  test("should return records with category structure when filtered by sex", async () => {
    const response = await authenticatedRequest().get("/api/records/raw/men");
    const category = response.body.data[0];
